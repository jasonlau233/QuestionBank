...
enum LinearStrokeCap { butt, round, roundAll, circle }
...
class LinearPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (isRTL) {
      ...
      if (linearStrokeCap == LinearStrokeCap.circle) {
        canvas.drawCircle(Offset(xProgress, size.height / 2),
            size.height / 2 + 1, circlePaint);
        canvas.drawCircle(Offset(xProgress, size.height / 2),
            size.height / 2 - 1, circlePaint..color = Colors.white);

        if (showTarget) {
          final xProgress2 = size.width * target;
          canvas.drawCircle(Offset(xProgress2, size.height / 2), 3,
              circlePaint..color = targetColor);
        }
      }
    }
  }
