...
class TestExercisesReportPage extends StatefulWidget {
  /// 标题
  final String title;
  ...
  @override
  _TestExercisesReportPageState createState() =>
      _TestExercisesReportPageState();
}

class _TestExercisesReportPageState extends State<TestExercisesReportPage> {
  ...
  void initState() {
    ...
    reportViewModel.pagerType = PagerType.EXAM;
              ...
              children: [
                CustomScrollView(
                  slivers: [
                    ...
                    ///分数信息
                    SliverToBoxAdapter(
                        child: _buildScoreInfo(model.reportEntity)),

                    ///答题分布
                    SliverToBoxAdapter(
                        child: _buildQuestionInfo(
                            model.arrangeCardEntity(model.reportCardEntity))),
  ...
  Future onModelReady(ReportViewModel model) async {
    ...
    bool success = await model.getReportInfos(
        widget.recordId, widget.paperUuid, quesIds,
              ...
              children: [
                ...
                CircularPercentIndicator(
                  ...
                  percent: reportViewModel.getPercent(
                    reportEntity.examScore,
                    reportEntity.paperScore,
                  ),
                  center: Column(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      Row(
                        ...
                        children: [
                          Text(
                            '${getScore(reportEntity.examScore)}',
                            style: TextStyle(
                                fontSize: 15, color: ColorUtils.color_bg_theme),
                          ),
                          Container(
                      ...
                      SizedBox(height: 3),
                      Text(
                        '总分${reportEntity.paperScore.toInt()}分',
                        style: TextStyle(
                            fontSize: 9, color: ColorUtils.color_text_level3),
                      )
                    ],
                  ),
                )
  ...
  ///分数信息
  Widget _buildScoreInfo(ReportEntity reportEntity) {
    return Container(
      height: 60,
      width: double.infinity,
        ...
        children: [
          Column(
            ...
            children: [
              ...
              SizedBox(height: 6),
              Text('${reportEntity.maxScore.toStringAsFixed(1) ?? 0}',
                  style: TextStyle(
                      fontSize: 12, color: ColorUtils.color_text_theme)),
            ],
          ),
          Container(
            width: 1,
          ...
          Column(
            ...
            children: [
              ...
              SizedBox(height: 6),
              Text('${reportEntity.avgScore.toStringAsFixed(1) ?? 0}',
                  style: TextStyle(
                      fontSize: 12, color: ColorUtils.color_text_choose_true)),
            ],
          ),
          Container(
            width: 1,
          ...
          Column(
            ...
            children: [
              ...
              SizedBox(height: 6),
              Text('${reportEntity.beatPersent.toStringAsFixed(1) ?? 0}%',
                  style: TextStyle(
                      fontSize: 12, color: ColorUtils.color_text_choose_fasle)),
            ],
          ),
        ],
      ),
  ...
  ///答题分布
  Widget _buildQuestionInfo(ReportCardEntity reportEntity) {
    ...
    return Container(
      color: Colors.white,
      width: double.infinity,
      padding: EdgeInsets.only(left: 12, right: 12, top: 24, bottom: 19),
      child: Column(
        children: [
          Row(
            children: [
          ...
          ListView.builder(
            ...
            itemBuilder: (_, index) {
              return _answerInfoItem(reportEntity.groups[index]);
            },
            itemCount: reportEntity.groups.length,
          ),
          Center(
  ...
  ///每个类型题目答题情况
  Widget _answerInfoItem(GroupsReport groupsReport) {
    return Container(
      height: 16,
      margin: EdgeInsets.only(top: 11, left: 3, right: 3),
      child: Row(
        children: [
          Container(
            width: 81,
            child: Text(
              '${reportViewModel.splitTitle(groupsReport.name)}',
              style: TextStyle(
                  fontSize: 12,
          ...
          LinearPercentIndicator(
            percent: reportViewModel.getPercent(
                groupsReport.rightCount.toDouble(),
                groupsReport.totalCount.toDouble()),
            lineHeight: 6,
            width: 148,
            backgroundColor: ColorUtils.color_textBg_choose_select_choose,
            progressColor: ColorUtils.color_bg_theme,
          ),
          Spacer(),
          Text(
            '做对${groupsReport.rightCount}',
            style: TextStyle(fontSize: 10, color: ColorUtils.color_text_theme),
          ),
          Text(
            '/${groupsReport.totalCount}题  ',
            style: TextStyle(fontSize: 10, color: ColorUtils.color_text_level3),
          ),
          Text(
            '得${getScore(groupsReport.score)}分',
            style: TextStyle(fontSize: 10, color: ColorUtils.color_text_level1),
          )
        ],
      ),
    );
  ...
  String getScore(double score) {
    List<String> scoreList = score.toString().split('.');
    if (scoreList[1] == '0') {
  ...
  void jumpToExec() {
    ...
    AppRouterNavigator.of(context).replace(
      ...
      params: {
        ...
        "subLibraryModuleId": widget.subModuleId,
