...
class DateConvertUtil {
  ...
  static String getHMSOfMilliseconds(int milliseconds) {
  ...
  ///返回当前时间戳
  static int getCurrentMillSeconds() {
    return DateTime.now().millisecondsSinceEpoch;
  }

  ///获取多少天之前的所有日期
  static List<String> getBeforeDatesStrList(int days,
      {DateFormat format = DateFormat.YEAR_MONTH_DAY,
      String dateSeparate = '-'}) {
    List<String> dates = [];
    for (int i = days - 1; i >= 0; i--) {
      dates.add(getDateStrByMillisecond(
          getCurrentMillSeconds() - i * oneDayMills,
          format: format,
